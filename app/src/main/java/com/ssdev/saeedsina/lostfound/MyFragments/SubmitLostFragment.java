...
public class SubmitLostFragment extends Fragment implements View.OnClickListener {

    private static final int READ_REQUEST_CODE = 42;
    LoadingDialog loadingDialog;

    private static final String TAG = "LOST_FRAGMENT_FRAGMENT";
    private static final int REQUEST_CODE_ASK_PERMISSIONS = 2;

    private Button btn_browse,btn_locationpicker,btn_lost_submit;
    private View myView;
    public EditText txt_date,txt_name,txt_desc,txt_reward,txt_tags,txt_contact;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        myView = inflater.inflate(R.layout.fragment_submit_lost, container, false);
        btn_lost_submit = (Button) myView.findViewById(R.id.btn_lost_submit);
        btn_lost_submit.setTag("submit");
        btn_lost_submit.setOnClickListener(this);
        txt_date = (EditText) myView.findViewById(R.id.txt_lost_date);
        txt_name = (EditText) myView.findViewById(R.id.txt_lost_name);
        txt_desc = (EditText) myView.findViewById(R.id.txt_lost_desc);
        txt_reward = (EditText) myView.findViewById(R.id.txt_lost_reward);
        txt_tags = (EditText) myView.findViewById(R.id.txt_lost_tags);
        txt_contact = (EditText) myView.findViewById(R.id.txt_lost_contact);
        ((MainActivity) getActivity()).setActionBarTitle("Submit New");

        btn_browse = (Button) myView.findViewById(R.id.btn_lost_imgpicker);
        ...
        btn_locationpicker = (Button) myView.findViewById(R.id.btn_lost_locationpicker);
        btn_locationpicker.setTag("location");

        selectedLocation = mMyApplication.getSelectedLostLocation();
        selectedImageUri = mMyApplication.getPicLostPath();
        draft = mMyApplication.getLostDraft();
        populateViews();
    ...
    private void populateViews() {
        if(draft != null){
            ...
            txt_tags.setText(mMyApplication.getLostTags());
            txt_reward.setText(draft.getReward());
            ...
            try {
                ...
                Picasso.with(getContext()).load(mMyApplication.getPicLostPath()).into(img);
        ...
        switch (view.getTag().toString()){
            ...
            case "location":
                Item draft=new Item(txt_date.getText().toString(),txt_desc.getText().toString(),selectedLocation,
                        true,txt_name.getText().toString(),"",txt_reward.getText().toString(),txt_contact.getText().toString());
                mMyApplication.setLostDraft(draft);
                mMyApplication.setPicLostPath(selectedImageUri);
                mMyApplication.setLostTags(txt_tags.getText().toString());
                mListener.onMenuItemSelected("location","lost",null);
                break;
            case "submit":
                Upload_Submit();
                break;
    ...
    public void Upload_Submit() {
        if (Validate()) {
            ...
            String desc = txt_desc.getText().toString();
            String date = txt_date.getText().toString();
            String reward = txt_reward.getText().toString();
            String contact = txt_contact.getText().toString();
            Item item2Send = new Item(date, desc, selectedLocation, true, name, "", reward,contact);
            ...
            new SaveOperation(item2Send,tags).execute();
        }
    }
    private class SaveOperation extends AsyncTask<Void, Void, String> {
        ...
        protected void onPostExecute(String result) {
            ...
            if(result.equals("OK")){
                ...
                mListener.onMenuItemSelected("menu","lost",null);
